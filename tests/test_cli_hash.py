...
def test_cli_hash_multi_files_multi_hashes(runner_factory):
    with runner_factory() as runner:
        cli_args = ["hash"]
        files = []
        hash_types = []
        hash_map = {}
        for (filename, hsh), hsh_value in TEST_FILES.items():
...
@pytest.mark.parametrize("filename,hsh", TEST_FILES.keys())
def test_cli_hash_multiple_hashtype(runner_factory, filename, hsh):
    hsh_value = TEST_FILES[(filename, hsh)]
    with runner_factory() as runner:
...
@pytest.mark.parametrize("filename,hsh", TEST_FILES.keys())
def test_cli_verify_hash_stdin(runner_factory, filename, hsh):
    hsh_value = TEST_FILES[(filename, hsh)]
    with runner_factory() as runner:
        result = runner.invoke(cli, ['hash', "-t", hsh, filename], catch_exceptions=False)
        common_asserts(result)
...
@pytest.mark.parametrize("filename,hsh", TEST_FILES.keys())
def test_cli_verify_hash_stdin_fail(runner_factory, filename, hsh):
    hsh_value = TEST_FILES[(filename, hsh)]
    with runner_factory() as runner:
        result = runner.invoke(cli, ['hash', "-t", hsh, filename], catch_exceptions=False)
        common_asserts(result)
